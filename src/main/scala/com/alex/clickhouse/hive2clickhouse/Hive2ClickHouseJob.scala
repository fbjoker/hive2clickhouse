...
//    val rename = isRename.toBoolean
    ...
    val conf = new SparkConf().setAppName("----------Hive2ClickHouseJob-----------")
    val spark = SparkSession.builder().config(conf).enableHiveSupport().getOrCreate()
    val context = spark.sparkContext

    val sqlDF = spark.sql(hql)
        ...
        while (rows.hasNext) {
          try {
            ...
            for (i <- 0 until colum_size_broadcast.value.intValue()) {
              println("pt_index_broadcast.getValue() is " + pt_index_broadcast.value)
